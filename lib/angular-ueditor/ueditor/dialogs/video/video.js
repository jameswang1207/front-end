...
(function(){
    ...
    function is_txurl(url){
        var result = {'txUrl':false};
        var pattern = /(\w+)=(\w+)/ig;
        var params = {};
        ...
        if(params['vid']) {
            ...
            var matchResult = url.match(/^http:\/\/v.qq.com\/cover\/.*\/(.*)\.html/);
            if(matchResult) {
                result.txUrl = true;
                result.vid = matchResult[1];
            }
        }
        return result;
    }
    ...
    function createPreviewVideo(url){
        ...
        var conUrl = is_txurl(url);
        if(conUrl.txUrl) {
            $G("preview").innerHTML = '' + 
                '<iframe style="position:relative; z-index:1;" height="280" width="548" frameborder="0"' +
                    ' src="http://v.qq.com/iframe/player.html?vid=' + conUrl.vid + '&width=548&height=280&auto=0" width=' + 548 + '&height=' + 280 + '&auto=0" allowfullscreen=""></iframe>';
        } else {
            $G("preview").innerHTML = '<div class="previewMsg"><span>'+lang.urlError+'</span></div>';
        }
